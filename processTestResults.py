...
for _, row in X.iterrows():
	...
	if all_routings is None:
		...
		for _, scenario in scenario_df.iterrows():
			...
			if scenario_identifier in pre_evaluated_scenarios.keys():
				print("  Metadata for scenario {} already evaluated!".format(scenario_id))
				scenario_metadata_dict[scenario_id] = pre_evaluated_scenarios[scenario_identifier]

			else:
				print("  Calculating new metadata for scenario {}".format(scenario_id))
				...
				meta_time = time.time() - t
				print("  Metadata for scenario {} calculated in {:.1f} seconds".format(scenario_id, meta_time))
